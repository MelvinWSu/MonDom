...
apikey = '9906f1086752397b75a022266d832d02e88c3025f30fb63d33864b8a939e53b1'
while True:
...
def DomainReportReader(domain, delay):
    ...
    if r.status_code == 200:
        try:
            jsonResponse = r.json()
            #print(jsonResponse)
            file = open('results.json', 'w+', newline='', encoding='utf-8')
            json.dump(jsonResponse, file)
            file.close()

            https = jsonResponse['https_certificate_date']
            siblings = jsonResponse['domain_siblings']
            downloadSamples = jsonResponse['detected_downloaded_samples']
            malwarebytes = jsonResponse['Malwarebytes hpHosts info']
            referrerSamples = jsonResponse['detected_referrer_samples']
            webutation = jsonResponse['Webutation domain info']
            #Opera = jsonResponse['Opera_domain_info']
            records = jsonResponse['dns_records']
            infectURL = jsonResponse['detected_urls']

            data = [https, siblings, downloadSamples, malwarebytes, referrerSamples, webutation, records, infectURL]# Opera, records, infectURL]
            print("in reporter")
            #print(data)
            return data
            ...
            print('There was an error when scanning {!s}. Adding domain to error list....'.format(domainSani))
            domainErrors.append(domainSani)
            print('in keyrerro reason "%s"' % str(e))
            return "keyError"
    # API TOS issue handling
    elif r.status_code == 204:
        print('Received HTTP 204 response. You may have exceeded your API request quota or rate limit.')
...
try:
    with open(filepath,
              'r') as infile:  # keeping the file open because it shouldnt# be opened/modified during reading anyway
        #print("before loop")
